...
function UserManagementPage() {
  ...
  const handleSave = async () => {
    try {
      if (isEditing) {
        // Gọi API update user tại đây nếu cần (chưa có thì giữ nguyên local update)
        setUsers(users.map(u => (u.id === currentUser.id ? currentUser : u)));
      } else {
        const userToCreate = { 
          ...
          avatar: currentUser.avatar // hoặc bỏ qua nếu backend chưa nhận avatar dạng base64/url
        };
